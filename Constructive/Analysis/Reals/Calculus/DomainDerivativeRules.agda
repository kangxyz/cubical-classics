...
module Constructive.Analysis.Reals.Calculus.DomainDerivativeRules where
...
private
  ...
  outer-scale-posInv-product-cancel :
    (κ ε η : ℚ⁺) →
    (posInv⁺ κ *⁺ ε) *⁺ (κ *⁺ η) ≡ ε *⁺ η
  outer-scale-posInv-product-cancel κ ε η =
...
derivativeWithinDomainConstantAtWith
  ...
  subst
    (BoundedByᶜ (ε *⁺ η))
    (sym
      (SolverHelpers.remainder-constant
        CauchyRealsCommRing
        c
        h))
...
derivativeWithinDomainChainAtWith
  ...
  scaledν≤ =
    subst
      ...
      (scale-posInv-cancel incrementScale (ν outerPrecision))
        ...
        (λ θ →
          BoundedByᶜ θ
            (withinDomainLinearRemainder
              ...
              increment
              outer-forward-domain))
        (outer-scale-posInv-product-cancel incrementScale halfε η)
        (f-derivative
          outerPrecision
          (incrementScale *⁺ η)
          scaledν≤
  ...
  scaled-inner-remainder-bound =
    subst
      ...
      (scale-posInv-product-cancel κ halfε η)
      (bounded-byᶜ-mul
        κ
        ((posInv⁺ κ *⁺ halfε) *⁺ η)
        df

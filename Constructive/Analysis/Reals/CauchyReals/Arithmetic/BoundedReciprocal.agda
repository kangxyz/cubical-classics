...
module Constructive.Analysis.Reals.CauchyReals.Arithmetic.BoundedReciprocal where
...
BoundedAwayPositiveᶜ : ℚ⁺ → ℝᶜ → Type₀
BoundedAwayPositiveᶜ ε x =
  rational (radius ε) ≤ᶜ x
    ...
    stepBound (κ , xκ) =
      ...
      stepApprox (q , x∼q) =
        ...
        fx∼fq =
          subst
            (λ ρ → f x ∼[ ρ ] fq)
            (scale-posInv-cancel L ηA)
            (snd (boundedReciprocalᶜ-lipschitz lowerBound)
              x (rational q) θA x∼qθA)

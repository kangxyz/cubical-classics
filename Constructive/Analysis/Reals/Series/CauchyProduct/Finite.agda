{- Finite algebra for Cauchy products of Cauchy-real sequences. -}
{-# OPTIONS --safe --lossy-unification #-}
module Constructive.Analysis.Reals.Series.CauchyProduct.Finite where
...
sequenceCauchyProduct-antiDiagonal :
  ...
  antiDiagonalSumᶜ (λ i j → u i ·ᶜ v j) n
sequenceCauchyProduct-antiDiagonal u v zero =
  sym (antiDiagonalSumᶜ-zero (λ i j → u i ·ᶜ v j))
sequenceCauchyProduct-antiDiagonal u v (suc n) =
  cong
    (u zero ·ᶜ v (suc n) +ᶜ_)
    (sequenceCauchyProduct-antiDiagonal
      (λ k → u (suc k))

...
module Constructive.Analysis.Reals.PowerSeries.DerivativeConvergence.StrictSubball.OnBall.Estimate where
...
abstract
  ...
  derivativeStrictSubballDroppedTerm≤Majorant :
    ...
    absᶜ
      (drop m (powerSeriesTerm (derivativePowerSeries a) h) j)
    ≤ᶜ
    drop m
      (derivativeStrictSubballFromOnBallMajorant {ρ = ρ} {σ = σ} ρ<σ)
      j
  derivativeStrictSubballDroppedTerm≤Majorant
      ...
      μ1≤sucm+j =
    transportAbs≤Left
      (drop-index m derivativeTerms j)
      (transport≤RightBack
        (drop-index m majorant j)
        (derivativeStrictSubballTermMajorizedFromProbeBoundOpaque
          {a = a}
          {ρ = ρ}
          {σ = σ}

...
module Constructive.Analysis.Reals.PowerSeries.Algebra.Finite where
...
shift-rationalMulLeft :
  (q : ℚ) →
  (u : ℕ → ℝᶜ) →
  (m : ℕ) →
  shift m (λ n → rational q ·ᶜ u n) ≡
  (λ n → rational q ·ᶜ shift m u n)
shift-rationalMulLeft q u zero =
  refl
shift-rationalMulLeft q u (suc m) =
  shift-rationalMulLeft q (λ n → u (suc n)) m
...
shift-mulLeft :
  ...
  shift m (λ n → x ·ᶜ u n) ≡
  (λ n → x ·ᶜ shift m u n)
shift-mulLeft x u zero =
  refl
shift-mulLeft x u (suc m) =
  shift-mulLeft x (λ n → u (suc n)) m
...
tailSum-mulLeft x u m k =
  cong
    (λ v → partialSum v k)
    (shift-mulLeft x u m) ∙
  partialSum-mulLeft x (shift m u) k
...
tailSum-rationalMulLeft q u m k =
  cong
    (λ v → partialSum v k)
    (shift-rationalMulLeft q u m) ∙
  partialSum-rationalMulLeft q (shift m u) k

...
module Constructive.Analysis.Reals.PowerSeries.Recenter.Base where
...
drop-recenterCoefficientTerm-at-zero-suc :
  (a : PowerSeries) →
  (n m : ℕ) →
  drop (suc m) (recenterCoefficientTerm a 0ᶜ n) ≡
  (λ _ → 0ᶜ)
drop-recenterCoefficientTerm-at-zero-suc a n m =
  funExt λ k →
    drop-index
      (suc m)
...
tailSum-recenterCoefficientTerm-at-zero a n m k (j , j+1≡m) =
  ...
  tail-at-suc =
    cong
      (λ u → partialSum u k)
      (drop-recenterCoefficientTerm-at-zero-suc a n j) ∙

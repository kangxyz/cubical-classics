...
module Constructive.Analysis.Reals.PowerSeries.Instances.Arctangent.GlobalAnalytic where
...
atanhᶜ-data-independent :
  (x : ℝᶜ) →
  (left right : SubunitBoundᶜ x) →
  atanhᶜ x left ≡ atanhᶜ x right
atanhᶜ-data-independent x left right =
  atanhᶜFromSubunitBound-data-independent
...
atanhᶜAnalyticWithinAt x domain =
  ...
  expansion =
    ...
    expansionPath y y-domain y-inBall =
      atanhᶜFromSubunitBound-data-independent
        (transformRadius y-domain)
        σ
        (transformRadius<1 y-domain)
        σ<1
        y
        (transformBound y-domain)
        y-bound
      ∙ sym

...
module Constructive.Analysis.Reals.PowerSeries.Elementary.Logarithm.Global where
...
LogTransformSubunitDataᶜ :
  ...
  BoundedAwayPositiveᶜ denomLower (x +ᶜ 1ᶜ) →
  Type₀
LogTransformSubunitDataᶜ x denomLower denomBound =
  StrictSubunitDataᶜ (logTransformᶜ x denomLower denomBound)


logDenominatorDomain :
  {x : ℝᶜ} →
...
logDenominatorLower :
  ...
  PositiveBoundedDomainᶜ x →
  ℚ⁺
logDenominatorLower domain =
  lower (logDenominatorDomain domain)


logDenominatorBound :
  {x : ℝᶜ} →
  (domain : PositiveBoundedDomainᶜ x) →
  BoundedAwayPositiveᶜ (logDenominatorLower domain) (x +ᶜ 1ᶜ)
logDenominatorBound domain =
  lowerBound (logDenominatorDomain domain)
...
logTransformSubunitDataFromPositiveBoundedᶜ x domain =
  ρ ,
  ρ<1 ,
  transform-bound
  where

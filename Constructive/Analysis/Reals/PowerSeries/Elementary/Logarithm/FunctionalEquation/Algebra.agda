...
module Constructive.Analysis.Reals.PowerSeries.Elementary.Logarithm.FunctionalEquation.Algebra where
...
open import
  Constructive.Analysis.Reals.PowerSeries.Elementary.Logarithm.FunctionalEquation.AtanhTransport
...
logOnePlusᶜWithinSubunitBall-global :
  ...
  let
    domain =
      onePlusStrictSubunitDomain ρ ρ<1 u u-bound
    transformData =
      logTransformSubunitDataFromPositiveBoundedᶜ (1ᶜ +ᶜ u) domain
  in
  logᶜ
    (1ᶜ +ᶜ u)
    (onePlusStrictSubunitDomain ρ ρ<1 u u-bound)
...
logOnePlusᶜWithinSubunitBall-global ρ ρ<1 u u-bound =
  cong (twoᶜ ·ᶜ_)
    (atanhᶜFromSubunitBound-argument-path
      ...
      (subunitBound transformData))
  where
  domain : PositiveBoundedDomainᶜ (1ᶜ +ᶜ u)
  domain =
    onePlusStrictSubunitDomain ρ ρ<1 u u-bound

...
module Constructive.Analysis.Reals.PowerSeries.Elementary.Logarithm.FunctionalEquation.LocalAdd where
...
private
  module LocalData
    ...
    Expected :
      ...
      ℝᶜ
    Expected t t-bound =
      logᶜ c c-domain +ᶜ LocalScaled t t-bound

    shiftedLogIndependent :
      DomainValueIndependent ShiftedLog
    shiftedLogIndependent t left right =
      cong (ShiftedLog t) (isPropBoundedByᶜ ρ t left right)

    expectedIndependent :
      DomainValueIndependent Expected
    expectedIndependent t left right =
      cong (Expected t) (isPropBoundedByᶜ ρ t left right)

    expectedDerivativeModulus : PrecisionModulus
    expectedDerivativeModulus ε =
      min⁺
        (exactLinearModulus (half⁺ ε))
      ...
      HasDerivativeWithinDomainAtWith
        ...
        (0ᶜ +ᶜ
          reciprocalPositiveᶜ
            ...
            (c +ᶜ t)
            (shiftWindow t t-bound .fst))
        expectedDerivativeModulus
    expectedDerivative t t-bound =
      derivativeWithinDomainAddAtWith
        (derivativeWithinDomainConstantAtWith
          {D = ShiftDomain}
          {c = logᶜ c c-domain}
          {x-domain = t-bound}
          {μ = exactLinearModulus})
        (localScaledDerivative t t-bound)
    ...
    shiftedLogIdentity :
      ...
      logᶜ c c-domain +ᶜ LocalScaled h h-bound
    shiftedLogIdentity h h-bound =
      sym shifted-endpoint-path ∙
      ...
      base-path :
        ShiftedLog 0ᶜ zero-bound ≡ Expected 0ᶜ zero-bound
      base-path =
        shiftedLogZero zero-bound ∙
        sym (add-zero-right (logᶜ c c-domain)) ∙
        cong
          (logᶜ c c-domain +ᶜ_)
          (sym (localScaledZero zero-bound))

      shifted-endpoint-path :
        ShiftedLog (0ᶜ +ᶜ h) forward-bound ≡
        ShiftedLog h h-bound
      shifted-endpoint-path =
        domainValueAlongPath
          shiftedLogIndependent
